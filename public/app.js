...
function saveShareLogin() {
  const name=document.getElementById("shareName").value.trim(), password=document.getElementById("sharePassword").value;
  if(name && password){localStorage.setItem("locationAccountName",name);localStorage.setItem("locationAccountPassword",password);}
}
(function(){const n=localStorage.getItem("locationAccountName"),p=localStorage.getItem("locationAccountPassword");
if(n) document.getElementById("shareName").value=n;if(p) document.getElementById("sharePassword").value=p;})();
async function sendLocation() {
  const name=document.getElementById("shareName").value.trim();
  const password=document.getElementById("sharePassword").value;
  if(!name || password.length<6){setStatus("shareStatus","Preencha nome e uma senha de pelo menos 6 caracteres.",true);return;} saveShareLogin();
  ...
  navigator.geolocation.getCurrentPosition(async pos=>{
    ...
    try{
      ...
      setStatus("shareStatus",`Localização atualizada. Expira em ${data.ttlMinutes} minutos.`);
